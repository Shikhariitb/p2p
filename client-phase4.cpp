        ...
        if(n_ids[j][1] < stoi(pid)){
            ...
            for(int k = 1; k < neigh_file_list[j].size(); k++){
                if(neigh_file_response[j][k] != "-1"){
                    string tmsg = neigh_file_list[j][k] + '~' + neigh_file_response[j][k];
            ...
            while(true){
                ...
                if(file_name != convertToString(flag, 0, 1)){
                    ...
                    ans[0] = 'y';
                    for (int i = 0; i < fc; i++){
                        if(flist[i][1] == "0" || stoi(flist[i][1]) > stoi(file_name.substr(di + 1,len - di - 1))){
                            if(flist[i][0] == file_name.substr(0,di)){
                                flist[i][1] = file_name.substr(di + 1,len - di - 1);
                                flist[i][2] = '2';
                            }
                        }
                    }
